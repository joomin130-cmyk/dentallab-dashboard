...
import {
  ChevronLeft, Save, Edit2, FileText,
...
const OrderDetailView = ({ order, onBack, onAssign }) => {
  ...
  const safeFuturePlan          = order.futurePlan          || '23년 12월 처음 셋팅하시고 깨져서 다시 제작, 24년 11월 셋팅 하신건데 깨져서 오심';
  ...
  return (
    /* h-full: App.jsx의 h-[calc(100vh-104px)] 컨테이너를 채움 */
    <motion.div
    ...
    >
      ...
      {/* ── Body: flex-1 min-h-0 필수 (flex children overflow 허용) ── */}
      <div className="flex gap-4 flex-1 min-h-0">
        ...
        <motion.div
          ...
          className="flex-1 min-w-0 bg-white rounded-[20px] overflow-y-auto custom-scrollbar"
        >
          ...
          <div className="px-8 py-6 border-b border-[#F2F4F6]">
            ...
            <div className="mb-5">
              <Banner variant="neutral" message={`보험 적용 : ${isInsured ? '예' : '아니오'}`} />
            </div>
            <div className="mb-5">
          ...
          <div className="px-8 py-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex bg-[#F2F4F6] p-1 rounded-[8px]">
                {[{ key: 'change', label: '의뢰 변경 이력' }, { key: 'patient', label: '환자 사전 정보' }].map(tab => (
                  <button
                    key={tab.key}
                    onClick={() => setActiveHistoryTab(tab.key)}
                    className={`px-3 py-1.5 text-[12px] font-bold rounded-[6px] transition-colors ${
                      activeHistoryTab === tab.key ? 'bg-white text-[#333D4B] shadow-sm' : 'text-[#8B95A1] hover:text-[#4E5968]'
                    }`}
                  >
                    {tab.label}
                  </button>
        ...
        <AnimatePresence>
          {showChat && (
            ...
            >
              {/* 고정 너비 내부 컨테이너 — 너비 클립 애니메이션 동안 내용 유지 */}
              <div style={{ width: 340 }} className="h-full flex flex-col">
                ...
                {/* 메시지 목록 */}
                <div className="flex-1 overflow-y-auto px-1 py-2 flex flex-col gap-4 custom-scrollbar min-h-0">
                  ...
                  {MOCK_MESSAGES.map((msg) => (
                    <div
                      key={msg.id}
                      className={`flex flex-col gap-1 ${msg.sender === 'lab' ? 'items-end self-end max-w-[88%]' : 'items-start max-w-[88%]'}`}
                    >
